...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
    ...
    echo "[planning-with-files] Before stopping, ensure progress.md is current and handoff.md summarizes the final state."
else
    echo "[planning-with-files] Task in progress ($COMPLETE/$TOTAL phases complete). Update progress.md before stopping."
    echo "[planning-with-files] If this session changed context, refresh handoff.md for the next agent."
...
if [ ! -f "handoff.md" ]; then
    echo "[planning-with-files] Missing handoff.md. Create it when preparing a handoff or updating planning files."
fi
exit 0
